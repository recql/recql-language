...
This repository contains the formal specification, EBNF grammar, Intermediate Representation (IR), execution semantics, and reference documentation for **RecQL (Recommender Query Language)**.
...
- **Formal EBNF Grammar**: Normative grammar specifying concrete statement syntax and IR-aligned function signatures with explicit parameter names for retrievers, encoders, scorers, and reorderers.
- **Intermediate Representation (IR)**: Complete schema, instruction set reference, and dual ingestion guide (RecQL SQL compilation vs. direct IR submission via JSON/YAML).
...
- **Cookbook & Query Recipes**: Production query templates for Semantic Search, Hybrid Search, Item-to-Item CF, Personalized Feeds, Promo Boosting, and Filter Bubble mitigation with interactive RecQL SQL and IR tabs.
...
- [recql-python-core](https://github.com/recql/recql-python-core): Core pure-Python RecQL parser, AST, lowering to IR, and engine runtime.
